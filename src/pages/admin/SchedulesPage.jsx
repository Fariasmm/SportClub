...
import { getSportRooms } from "../../services/sportRoomService" // <-- Importamos las asignaciones para cruzar los datos

function SchedulesPage() {
  const [schedules, setSchedules] = useState([])
  const [assignments, setAssignments] = useState([]) // <-- Guardamos la lista de asignaciones reales
  ...
  const loadSchedulesAndRelations = async () => {
    try {
      setLoading(true)
      // Ejecutamos ambas peticiones en paralelo para asegurar el cruce
  ...
  // =========================================================================
  // ⚡ LÓGICA DE CRUCE LOCAL (CRUZA IDs CON EL ARREGLO DE ASIGNACIONES REALES)
  // =========================================================================
  const findLinkedAssignment = (sc) => {
    if (!sc) return null
    // Busca la asignación que calce con el sport_room_id de la clase actual
  ...
  const getSportName = (sc) => {
    ...
    // Si el backend sí mandó la relación, la dejamos de respaldo
            ...
            <div className="table-responsive">
              <Table variant="dark" hover className="m-0 align-middle" style={{ backgroundColor: colors.purple }}>
                ...
                <tbody>
                  {schedules.length === 0 ? (
                    <tr>
                      <td colSpan="7" className="text-center py-4 text-white-50">No hay bloques horarios establecidos para esta semana.</td>
                    </tr>
                  ) : (
                    schedules.map((sc) => (
                      <tr key={sc.id} style={{ borderBottom: `1px solid ${colors.inputBorder}` }}>
                        <td className="py-3 px-4 fw-bold text-white-50">#{sc.id}</td>
                        <td className="py-3 fw-bold text-light">{sc.day_of_week}</td>
                        <td className="py-3">
